...
| 维度 | 当前值 | 含义 |
|---|---|---|
| 标题翻译 / 知识源全文翻译 | Radar 默认 DeepSeek，失败后 Google Translate | `DEEPSEEK_API_KEY`、`DEEPSEEK_MODEL` 控制首选模型；`GOOGLE_TRANSLATE_ENDPOINT` 配置兜底服务；已注册知识源统一走 Radar TranslationRouter |
